<template>
  <b-container>
    <div id="grouptwocomponent" class>
      <b-row
        class="Clientzero"
...
export default {
  ...
  mounted() {
    if (this.socket) {
      this.socket.on("group2", (data) => {
         var count = ++this.$notifications.group2;
        this.$notifications.group2 =count;
        var audio = new Audio(
      ...
      this.socket.on("group2userjoined", (data) => {
        if (data) {
          var audio = new Audio(
            "http://soundbible.com/mp3/Elevator Ding-SoundBible.com-685385892.mp3"
          );
          audio.play();
          if (document.getElementById("grouptwocomponent")) {
            ...
            document.getElementById("grouptwocomponent").innerHTML +=
              '<div id="client-left" class="row no-gutters justify-content-center">' + data.name + '   Joined !</div>';
          }
        }
      });
    }
